...
# It takes the entire environment instead of several training parameters X and Y as the parameter.

# RL can be used in psychology and predicting animal behavior.

# SL focuses on increase performance and lower the cost. 
# Unlike SL and UL, RL focuses on getting a message back from the environment, and this process should be autometic.
# In SL, labels need to be manually added so that the algorithm can be trained, but in RL, we train the algorithm based
#   on the feedback from the environment. 



# Terminologies
# 1. Agent: The object that lives in the environment. The object into which we are trying to code AI.
...
# 4. Rewards: The feedback from the environment. Positive reward should be given if AI performs expectedly, otherwise, negative reward
#   should be given.
